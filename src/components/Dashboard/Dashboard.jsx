...
import { Target, TrendingUp, Zap, Flame, Award } from 'lucide-react'
...
function CircularProgress({ percentage, colorClass, strokeColor, label }) {
  const radius = 15.9155; // Mathematical trick: 2 * pi * r = 100 circumference
...
export default function Dashboard() {
  ...
  // Funnel Math for the CSS Bar Chart
  const maxFunnel = Math.max(goals.knocks, 1) // prevent division by zero
  const appBarWidth = Math.max((goals.apps / maxFunnel) * 100, 2) // min 2% so it's visible
